...
def corre_vcap_73
  ...
  globais = ["Termo Potencia Contratada (eur)", "Termo Energia (eur)"]
  categoria = [["Custo Potencia Contratada (eur)"],
               ["Total Termo Ponta (eur)", "Custo Cheia (eur)",
                "Custo Vazio (eur)", "Custo Super-Vazio (eur)"]]
  tabela = []
  tabela_somatorio = [["Categoria","Valor (eur)"]]
  for cat in 0...categoria.size
    colunas = []
    for subcat in 0...categoria[cat].size
      for i in 0...sites.size
        nome = "timestral_#{sites[i]}"
        nome = ReportChapter7a.new(sites[i], categoria[cat][subcat])
        colunas[i] = nome.custo_trimestral
      end
    tabela[cat] = ReportChapter7a.concatena_tabelas(colunas, categoria[cat][subcat], globais[cat])
    end
    tabela = ReportChapter7a.cria_tabela
    tabela = Tabelas.calcula_soma_colunas(tabela, 1)
    tabela_somatorio.push([globais[cat], tabela[-1][-1]])
    ReportChapter7a.limpa_tabela
  end
  tabela_somatorio = Tabelas.calcula_soma_colunas(tabela_somatorio, 1)
  Ficheiro.cria_csv(tabela_somatorio, ReportChapter7a.nome_ficheiro_gerar, 1)
end
