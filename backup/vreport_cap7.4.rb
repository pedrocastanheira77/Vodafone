...
class ReportChapter7b
  ...
  def self.nome_ficheiro_gerar(tipo)
    "report/cap_7.4_#{tipo}.csv"
  end
...
def corre_vcap_74(tipo, globais, categoria, sites)
  tabela = []
  ...
  for cat in 0...categoria.size
    ...
    for subcat in 0...categoria[cat].size
      for i in 0...sites.size
        ...
        nome = ReportChapter7b.new(sites[i], categoria[cat][subcat])
        colunas[i] = nome.custo_trimestral
      end
    tabela[cat] = ReportChapter7b.concatena_tabelas(colunas, categoria[cat][subcat], globais[cat])
    end
    tabela = ReportChapter7b.cria_tabela
    tabela = Tabelas.calcula_soma_colunas(tabela, 1)
    tabela_somatorio.push([globais[cat], tabela[-1][-1]])
    ReportChapter7b.limpa_tabela
  end
  tabela_somatorio = Tabelas.calcula_soma_colunas(tabela_somatorio, 1)
  Ficheiro.cria_csv(tabela_somatorio, ReportChapter7b.nome_ficheiro_gerar(tipo), 1)
end

def corre_vcap_74_consumo_energia
  ...
  for i in 0...tipo.size
    corre_vcap_74(tipo[i], globais[i], categoria[i], sites)
  end
end
