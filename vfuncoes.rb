...
class GetConsumoPeriodoTarifario
  def self.consumo_periodo_tarifario(cabecalho, periodo, dados)
    tabela = [[cabecalho]]
    for mes in 1..12
      soma =[]
      soma_mes = 0
      for i in 1...dados.size
        condicao = (dados[i][2].sub(/,/,".").to_f == mes) && (dados[i][7] == periodo)
        if condicao == true
...
class GetCosts
  def self.calcula_custos_periodo(cabecalho, periodo, dados)
      tabela = [[cabecalho]]
      for mes in 1..12
        soma =[]
        soma_mes = 0
        for i in 1...dados.size
          if (dados[i][2].sub(/,/,".").to_f == mes && dados[i][7] == periodo)
...
class GetIndex
  def self.linhas(texto, tabela)
    for index in 0...tabela.size
      break if tabela[index][0].include?(texto)
    end
    index
  end
end
...
class TotalColuna
  ...
  def self.calcula_acumulado_por_coluna(coluna)
    num_indicadores = coluna[0].size
    num_linhas = coluna.size
    acumulado = Array.new(num_indicadores,0)
    for i in 1...num_linhas
      for j in 0...num_indicadores
        acumulado[j] += coluna[i][j]
...
class AgrupamentoInformaçãoTrimestral

  def self.calcula_acumulado_trimestre(acumula_trimestre, colunas)
    for i in 1...colunas.size
      acumulado = acumula_trimestre[i][0] + colunas[i][0]
      acumula_trimestre[i][0] = acumulado.round(2)
    end
    acumula_trimestre_somatorio = Array.new(acumula_trimestre)
    acumula_trimestre_somatorio.pop          # Não se pode somar a linha do somatório por isso tem de se descatar a última linha
    acumula_trimestre[5][0] = TotalColuna.calcula_acumulado_por_coluna(acumula_trimestre_somatorio)[0]
    acumula_trimestre
  end
end

class AgrupamentoInformaçãoAnaul

  def self.calcula_acumulado_trimestre(acumula_ano, colunas)
    for i in 1...colunas.size
      acumulado = acumula_ano[i][0] + colunas[i][0]
      acumula_ano[i][0] = acumulado.round(2)
    end
    acumula_ano_somatorio = Array.new(acumula_ano)
    acumula_ano_somatorio.pop          # Não se pode somar a linha do somatório por isso tem de se descatar a última linha
    acumula_ano[5][0] = TotalColuna.calcula_acumulado_por_coluna(acumula_ano_somatorio).round(2)
    acumula_ano
  end
end
